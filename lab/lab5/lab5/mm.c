...
/* A BlockInfo contains information about a block, including the size
   ...
   needed when the block is free(allocated block doesn't have footer).
   To achieve better utilization, mm_malloc
...
/* SIZE(blockInfo->sizeAndTags) extracts the size of a 'sizeAndTags' field.
   ...
   (ALIGNMENT - 1) = TAG_USED+TAG_PRECEDING_USED=000...0111
   ～7=111...1000
*/
...
void putSizeAndTags(void *ptr, size_t sizeAndTags){
  ...
  *(size_t *)UNSCALED_POINTER_ADD(blockInfo, SIZE(blockInfo->sizeAndTags) - WORD_SIZE) = sizeAndTags; 
}
...
int mm_init() {
  ...
  // preceding is assumed to be used since this is 1st block!
...
void* mm_malloc(size_t size) {
  ...
  size += WORD_SIZE;
  // Yukun: reqsize is the actual block size allocated
  ...
  // Yukun idea from https://www.youtube.com/watch?v=pBN3WlvZFcU
  while(1) {
    ptrFreeBlock = (BlockInfo*) searchFreeList(reqSize);
    // if the ptrfreeblock is found, it then can be allocated
    if (ptrFreeBlock != NULL) {
      size_t remainSize = SIZE(ptrFreeBlock->sizeAndTags) - reqSize;
      // we should check if the remaining size of the preFreeBlock is enough
      // meaning >=MIN_BLOCK_SIZE, otherwise, the remaining block should not
      // be added into the free list
      // fprintf(stderr, "ptrFreeBlock size: %ld, reqSize: %ld, remaining size: %ld, MIN_BLOCK_SIZE: %ld",
      // SIZE(ptrFreeBlock->sizeAndTags), reqSize, remainSize);
      if (remainSize >= MIN_BLOCK_SIZE) {
        // header for the allocated block
        putSizeAndTags(ptrFreeBlock, reqSize | TAG_PRECEDING_USED | TAG_USED);
        // the remainFreeBlock created
        BlockInfo* remainFreeBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
        putSizeAndTags(remainFreeBlock, remainSize | TAG_PRECEDING_USED);
        // the remainFreeBlock is added into the free list
        // we should change the pointer and redirect next,prev pointer
        // handling prev ptrs
        if (FREE_LIST_HEAD == ptrFreeBlock) {
          FREE_LIST_HEAD = remainFreeBlock;
          remainFreeBlock->prev = NULL;
        } else {
          remainFreeBlock->prev = ptrFreeBlock->prev;
          ptrFreeBlock->prev->next = remainFreeBlock;
        }
        // handling next ptrs
        remainFreeBlock->next = ptrFreeBlock->next;
        if (ptrFreeBlock->next != NULL) {
          ptrFreeBlock->next->prev = remainFreeBlock;
        }
      } else {
        // remainSize < MIN_BLOCK_SIZE, followingBlock is not part of free list, it's not linked
        putSizeAndTags(ptrFreeBlock, ptrFreeBlock -> sizeAndTags | TAG_PRECEDING_USED | TAG_USED);
...
void mm_free(void *ptr) {
  ...
  // if the ptr is already freed, return null
  blockInfo = (BlockInfo *)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
  if ((blockInfo->sizeAndTags & TAG_USED) == 0) {
    return NULL;
  }

  // add 0 TAG_USED sizeAndTag for blockinfo and
  // 0 TAG_PRECEDING_USED for following block
